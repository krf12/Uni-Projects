...
var sunVenusCopyMatrix = mat4.create();
mat4.identity(sunVenusCopyMatrix);
var venusAngle = 0;
var venusRadius = 0;
var sigAngle = 0;

function initVenusDrawing(matrix){
  var orEccentricity = 0.0067;
  var orRadius = 30.8;
  var angVelocity = 0.0299;

  mat4.identity(matrix);
  mat4.translate(matrix, [0,0,0]);
  mat4.multiply(matrix, sunVenusCopyMatrix);

  mat4.multiply(matrix, sunVenusCopyMatrix, venusRotationMatrix);

  mat4.identity(matrix);

  venusRadius = (orRadius*(1 + orEccentricity))/(1 + orEccentricity*Math.cos(venusAngle));
  sigAngle = 0.5* orRadius * orRadius * angVelocity/(venusRadius*venusRadius);
  venusAngle += sigAngle;

  var orRotationMatrix = mat4.create();
  mat4.identity(orRotationMatrix);
  mat4. translate(orRotationMatrix, [venusRadius * Math.sin(venusAngle), 0, venusRadius*Math.cos(venusAngle)]);
  mat4.multiply(orRotationMatrix, venusRotationMatrix, venusRotationMatrix);
