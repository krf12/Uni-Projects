...
function initMercuryDrawing(matrix){
  var orEccentricity = 0.21;
  var orRadius = 20.8;
  var angVelocity = 0.125;

  mat4.identity(matrix);

  var sunCopyMatrix = mat4.create();
  mat4.identity(sunCopyMatrix);

  mat4.translate(matrix, [0,0,0]);
  mat4.multiply(matrix, sunCopyMatrix);

  mat4.multiply(matrix, sunCopyMatrix, mercuryRotationMatrix);

  var orRotationMatrix = mat4.create();
  mat4.identity(orRotationMatrix);
  //mercuryAngle = mercuryAngle + 0.01;

  mercuryRadius = (orRadius*(1 + orEccentricity))/(1 + orEccentricity*Math.cos(mercuryAngle));
  sigAngle = 0.5* orRadius * orRadius * angVelocity/(mercuryRadius*mercuryRadius);
  mercuryAngle += sigAngle;

  mat4. translate(orRotationMatrix, [mercuryRadius * Math.sin(mercuryAngle), 0, mercuryRadius*Math.cos(mercuryAngle)]);
  mat4.multiply(orRotationMatrix, mercuryRotationMatrix, mercuryRotationMatrix);
